...
class MultiLabelDataset(Dataset):
    ...
    def __init__(self,
                 input_dataset: Dataset,  # The input Dataset is of type NamedTensorDataset
                 labels: List[Set[int]],
                 label_pool: Set[int],
                 num_candidates: int
                 ) -> None:
        ...
        num_candidates = len(
            label_pool) if num_candidates is None else num_candidates
        ...
        assert n == m, "Labels (%i) and Input Dataset (%i) do not align!" % (
            n, m)
        assert all(
    ...
    def sample_candidates(self, index: int, positives: Set[int]) -> Set[int]:
        ...
        positives = sample(positives, k=min(
            len(positives), self.num_candidates//2))
        negatives = sample(
...
class GroupWeightedMultiLabelDataset(MultiLabelDataset):
    ...
    def sample_candidates(self, index: int, positives: Set[int]) -> Set[int]:
        ...
        negative_groups = torch.LongTensor(
            [g for g in self.groups if g not in positive_groups])
